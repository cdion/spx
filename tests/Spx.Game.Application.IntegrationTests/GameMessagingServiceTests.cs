    ...
    : IntegrationTestBase(fixture)
{
    ...
    public async Task PersistResolvedBatchAsync_CreatesPublicGameplayMessagesOncePerResolution()
    {
        ...
        var firstPlayer = await database.Context.GamePlayers.SingleAsync(entry =>
            entry.GameId == game.Id && entry.UserId == "user-1"
        );
        var secondPlayer = await database.AddGamePlayerAsync(game.Id, "user-2", "Captain Blue");
        var writer = new EfGameplayEventMessageWriter(
            database.ContextFactory,
            new GameplayEventMessageFormatter()
        );
        ...
        var (lastResolvedBatch, completion) = CreateResolvedBatch(
            firstPlayer.Id,
            secondPlayer.Id,
            resolvedAtUtc
        );
        var persistedCount = await writer.PersistResolvedBatchAsync(
            game.Id,
            lastResolvedBatch,
            completion,
            gameplayEvents
        );
        var persistedCountOnRetry = await writer.PersistResolvedBatchAsync(
            game.Id,
            lastResolvedBatch,
            completion,
            gameplayEvents
        ...
        var messages = await database
            .Context.GameMessages.Where(entry => entry.Kind == GameMessageKind.GameplayEvent)
        ...
        Assert.Contains(
            messages,
            entry => entry.Body.Contains("Round 4 resolved.", StringComparison.Ordinal)
        );
        Assert.Contains(
            messages,
            entry =>
                entry.Body.Contains(
                    "Captain Red won by producing Victory.",
    ...
    private static (GameResolvedBatchView, GameCompletionView) CreateResolvedBatch(
        Guid firstPlayerId,
        Guid secondPlayerId,
    ...
    )
    {
        ...
        var completion = new GameCompletionView(
            GameCompletionReason.Victory,
            firstPlayer,
            resolvedAtUtc
        );
