...
public sealed class LeaveGameHandlerTests
{
    ...
    public async Task HandleAsync_publishes_lobby_and_messages_when_leave_changes_state()
    {
        ...
        persistence.LeaveGameAsync(Arg.Any<Guid>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
        ...
        await persistence.Received(1).LeaveGameAsync(gameId, "user-1", Arg.Any<CancellationToken>());
        await sessionService.Received(1).AbandonAsync(gameId, playerId, Arg.Any<CancellationToken>());
    ...
    public async Task HandleAsync_does_not_abandon_or_publish_when_leave_result_is_unchanged()
    {
        ...
        var persistence = Substitute.For<IGamePersistence>();
        persistence.LeaveGameAsync(Arg.Any<Guid>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns(new LeaveGamePersistenceResult(new GameCommandFailed("You are not an active player in this game.")));
        var sessionService = Substitute.For<IGameSessionService>();
        var lobbyPublisher = Substitute.For<IGameLobbyInvalidationPublisher>();
        var messagePublisher = Substitute.For<IGameMessageInvalidationPublisher>();
        using var services = CreateServices(persistence, sessionService, lobbyPublisher, messagePublisher);
        ...
        await sessionService.DidNotReceive().AbandonAsync(Arg.Any<Guid>(), Arg.Any<Guid>(), Arg.Any<CancellationToken>());
