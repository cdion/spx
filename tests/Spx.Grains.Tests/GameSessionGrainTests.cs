...
public sealed class GameSessionGrainTests
{
    ...
    {
        var state = CreateInitializedState();
        var firstPicker = state.CurrentAcquireFirstUserId == FirstPlayer.UserId ? FirstPlayer : SecondPlayer;
        var pickedCard = state.VisibleMarketCards[0];

        var view = GameSessionEngine.SubmitAcquire(
            state,
            GameId,
            new SubmitAcquireCardCommand(firstPicker.UserId, state.RoundNumber, pickedCard.CardInstanceId));

        Assert.Equal(GamePhase.Acquire, view.Phase);
        Assert.True(view.WaitingForOpponent);
        Assert.Equal(4, state.VisibleMarketCards.Count);
        Assert.Contains(GetHand(state, firstPicker.UserId), card => card.CardInstanceId == pickedCard.CardInstanceId);
    }
    ...
    {
        var state = CreateInitializedState();
        var firstPickerUserId = state.CurrentAcquireFirstUserId!;
        var secondPickerUserId = state.CurrentAcquireSecondUserId!;

        GameSessionEngine.SubmitAcquire(
            state,
            GameId,
            new SubmitAcquireCardCommand(firstPickerUserId, state.RoundNumber, state.VisibleMarketCards[0].CardInstanceId));

        GameSessionEngine.SubmitAcquire(
            state,
            GameId,
            new SubmitAcquireCardCommand(secondPickerUserId, state.RoundNumber, state.VisibleMarketCards[0].CardInstanceId));

        var nextView = GameSessionEngine.GetSessionView(state, GameId, new GetGameSessionViewQuery(firstPickerUserId));
    ...
    public void SubmitAcquire_enters_play_phase_after_four_acquire_picks()
    {
        ...
        {
            var currentUserId = GetCurrentAcquireUserId(state);
            GameSessionEngine.SubmitAcquire(
                state,
                GameId,
                new SubmitAcquireCardCommand(currentUserId, state.RoundNumber, state.VisibleMarketCards[0].CardInstanceId));
        }
    ...
    public void SubmitPlayBatch_consumes_extract_outputs_in_same_batch_for_refine()
    {
        ...
        GameSessionEngine.SubmitPlayBatch(
            ...
            new SubmitPlayBatchCommand(
                ...
                state.RoundNumber,
                [
                    new GameBatchCardCommand(firstExtract.CardInstanceId, GameResourceColor.Red, null, null, null, []),
                    new GameBatchCardCommand(secondExtract.CardInstanceId, GameResourceColor.Blue, null, null, null, []),
                    new GameBatchCardCommand(
                        ...
                        null,
                        [
                            new GameCardReferenceCommand(null, firstExtract.CardInstanceId, GameCardDefinition.Red),
                            new GameCardReferenceCommand(null, secondExtract.CardInstanceId, GameCardDefinition.Blue)
                        ])
                ]),
            DateTime.UtcNow);

        var secondPlayerResult = GameSessionEngine.SubmitPlayBatch(
            state,
            GameId,
            new SubmitPlayBatchCommand(SecondPlayer.UserId, state.RoundNumber, []),
            DateTime.UtcNow);
    ...
    public void SubmitPlayBatch_allows_incomplete_refine_and_fizzles_during_resolution()
    {
        ...
        Assert.Equal(2, secondPlayerResult.Session.RoundNumber);
        Assert.Contains(secondPlayerResult.GameplayEvents, entry => entry.Contains("Refine fizzled", StringComparison.Ordinal));
    ...
    public void SubmitPlayBatch_allows_incomplete_extract_and_fizzles_during_resolution()
    {
        ...
        Assert.Contains(secondPlayerResult.GameplayEvents, entry => entry.Contains("Extract fizzled", StringComparison.Ordinal));
    ...
    public void SubmitPlayBatch_allows_incomplete_produce_and_fizzles_during_resolution()
    {
        ...
        Assert.Contains(secondPlayerResult.GameplayEvents, entry => entry.Contains("Produce fizzled", StringComparison.Ordinal));
    ...
    public void SubmitPlayBatch_allows_missing_sabotage_target_and_fizzles_during_resolution()
    {
        ...
        Assert.Contains(secondPlayerResult.GameplayEvents, entry => entry.Contains("Sabotage fizzled", StringComparison.Ordinal));
    }
    ...
    public void SubmitPlayBatch_allows_missing_replicate_target_and_fizzles_during_resolution()
    {
        ...
        Assert.Contains(secondPlayerResult.GameplayEvents, entry => entry.Contains("Replicate fizzled", StringComparison.Ordinal));
    }
    ...
    public void SubmitPlayBatch_allows_incomplete_catalyst_and_fizzles_during_resolution()
    {
        ...
        GameSessionEngine.SubmitPlayBatch(
            ...
            new SubmitPlayBatchCommand(
                FirstPlayer.UserId,
                state.RoundNumber,
                [new GameBatchCardCommand(catalyst.CardInstanceId, null, null, null, red.CardInstanceId, [])]),
            DateTime.UtcNow);
        ...
        Assert.Contains(secondPlayerResult.GameplayEvents, entry => entry.Contains("Catalyst fizzled", StringComparison.Ordinal));
        Assert.Contains(state.FirstPlayerHand, card => card.CardInstanceId == red.CardInstanceId);
    }
    ...
    public void SubmitPlayBatch_allows_missing_corrupt_target_and_fizzles_during_resolution()
    {
        ...
        Assert.Contains(secondPlayerResult.GameplayEvents, entry => entry.Contains("Corrupt fizzled", StringComparison.Ordinal));
    }
    ...
    public void SubmitPlayBatch_allows_missing_reclaim_target_and_fizzles_during_resolution()
    {
        ...
        Assert.Contains(secondPlayerResult.GameplayEvents, entry => entry.Contains("Reclaim fizzled", StringComparison.Ordinal));
    }
    ...
    private static List<GameCardState> GetHand(GameSessionGrainState state, string userId)
        => userId == FirstPlayer.UserId ? state.FirstPlayerHand : state.SecondPlayerHand;
