...
public sealed class GamePresenceGrainIntegrationTests(OrleansClusterFixture fixture)
{
    ...
    public async Task PresenceExpiry_publishes_invalidation_without_a_read()
    {
        ...
        var observerReference = fixture.Cluster.Client.CreateObjectReference<IGameInvalidationObserver>(observer);
        ...
        try
        {
            await presenceGrain.UpsertLeaseAsync(new UpsertGamePresenceLeaseCommand(
                playerId,
                connectionId,
            ...
            var invalidated = await observer.TryWaitForPresenceInvalidationAsync(TimeSpan.FromSeconds(8));
            var snapshot = await presenceGrain.GetSnapshotAsync();

            Assert.True(invalidated, $"Expected background expiry invalidation before timeout, but final online players were [{string.Join(", ", snapshot.OnlinePlayerIds)}].");
            Assert.Empty(snapshot.OnlinePlayerIds);
        }
        finally
        {
            await invalidationGrain.Unsubscribe(observerReference);
            fixture.Cluster.Client.DeleteObjectReference<IGameInvalidationObserver>(observerReference);
        }
    }
    ...
    public async Task RemoveLease_publishes_invalidation_through_orleans()
    {
        ...
        try
        {
            ...
            await presenceGrain.RemoveLeaseAsync(new RemoveGamePresenceLeaseCommand(playerId, connectionId));

            var invalidated = await observer.TryWaitForPresenceInvalidationAsync(TimeSpan.FromSeconds(2));
            var snapshot = await presenceGrain.GetSnapshotAsync();

            Assert.True(invalidated, "Expected presence invalidation after removing the last lease.");
            Assert.Empty(snapshot.OnlinePlayerIds);
        }
        finally
        {
    ...
    public async Task PresenceExpiry_does_not_publish_invalidation_when_another_connection_keeps_player_online()
    {
        ...
        try
        {
            ...
            Assert.False(invalidated, "Did not expect presence invalidation while another connection still keeps the player online.");
    ...
    public async Task RemoveLease_does_not_publish_invalidation_when_another_connection_keeps_player_online()
    {
        ...
        try
        {
            ...
            await presenceGrain.RemoveLeaseAsync(new RemoveGamePresenceLeaseCommand(playerId, removedConnectionId));
    ...
    {
        private readonly TaskCompletionSource<Guid> completionSource = new(TaskCreationOptions.RunContinuationsAsynchronously);
