...
public sealed class AccountLinkBuilderTests
{
    ...
    public void BuildPasswordResetLink_uses_query_encoding_and_preserves_base_path()
    {
        var builder = new AccountLinkBuilder(
            Microsoft.Extensions.Options.Options.Create(
                new AppUrlOptions { BaseUrl = "https://example.com/spx" }
            )
        );
        ...
        Assert.Equal(
            "https://example.com/spx/reset-password?email=user%2Btest@example.com&code=code%20value",
            link
        );
    }
    ...
    public void Constructor_throws_when_base_url_is_missing()
    {
        var exception = Assert.Throws<InvalidOperationException>(() =>
            new AccountLinkBuilder(Microsoft.Extensions.Options.Options.Create(new AppUrlOptions()))
        );
