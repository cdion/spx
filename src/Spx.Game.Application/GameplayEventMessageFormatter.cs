...
public sealed class GameplayEventMessageFormatter : IGameplayEventMessageFormatter
{
    ...
    )
    {
        ...
        var summaryLines = new List<string> { $"Round {lastResolvedBatch.RoundNumber} resolved." };
        ...
        var messages = new List<string> { string.Join("\n", summaryLines) };
    ...
    private static string FormatGameplayEvent(
        ...
        gameplayEvent.Kind switch
        {
            GameplayEventKind.Fizzled =>
                $"{ResolvePlayerName(gameplayEvent.ActorPlayerId, playerNames)}'s {gameplayEvent.SourceCardDefinition} fizzled.",
            GameplayEventKind.DiscardedCard =>
                $"{ResolvePlayerName(gameplayEvent.ActorPlayerId, playerNames)} resolved {gameplayEvent.SourceCardDefinition} and discarded {gameplayEvent.TargetCardDefinition} from {ResolvePlayerName(gameplayEvent.TargetPlayerId, playerNames)}.",
            GameplayEventKind.CreatedCard => FormatCreatedEvent(gameplayEvent, playerNames),
            GameplayEventKind.ConvertedCard =>
                $"{ResolvePlayerName(gameplayEvent.ActorPlayerId, playerNames)} resolved {gameplayEvent.SourceCardDefinition} and converted {gameplayEvent.TargetCardDefinition} into {gameplayEvent.ProducedCardDefinition}.",
            GameplayEventKind.ScheduledReturnToHand =>
                $"{ResolvePlayerName(gameplayEvent.ActorPlayerId, playerNames)} resolved {gameplayEvent.SourceCardDefinition} and will return {gameplayEvent.TargetCardDefinition} to hand.",
            GameplayEventKind.ReturnedToHand =>
                $"{ResolvePlayerName(gameplayEvent.ActorPlayerId, playerNames)} returned {gameplayEvent.SourceCardDefinition} to hand.",
            GameplayEventKind.Resolved =>
                $"{ResolvePlayerName(gameplayEvent.ActorPlayerId, playerNames)} resolved {gameplayEvent.SourceCardDefinition}.",
            _ => throw new InvalidOperationException("Unknown gameplay event kind."),
        };

    private static string FormatCreatedEvent(
        GameplayEvent gameplayEvent,
        IReadOnlyDictionary<Guid, string> playerNames
    )
    {
        ...
        return gameplayEvent.SourceCardDefinition switch
        {
            GameCardDefinition.Extract =>
                $"{actorName} extracted {gameplayEvent.ProducedCardDefinition}.",
            GameCardDefinition.Refine =>
                $"{actorName} refined {gameplayEvent.ProducedCardDefinition}.",
            GameCardDefinition.Produce =>
                $"{actorName} produced {gameplayEvent.ProducedCardDefinition}.",
            _ =>
                $"{actorName} resolved {gameplayEvent.SourceCardDefinition} and created {gameplayEvent.ProducedCardDefinition}.",
        };
    }

    private static string CreateCompletionBody(
        GameCompletionView completion,
        IReadOnlyDictionary<Guid, string> playerNames
    ) =>
