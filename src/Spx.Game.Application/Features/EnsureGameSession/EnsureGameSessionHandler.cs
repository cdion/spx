...
internal sealed class EnsureGameSessionHandler(
    ...
    IGameSessionService gameSessionService) : IEnsureGameSessionHandler
{
    public async Task<bool> HandleAsync(Guid gameId, CancellationToken cancellationToken = default)
    {
        ...
        return await gameSessionService.EnsureSessionAsync(gameId, activePlayers, cancellationToken);
    }
}