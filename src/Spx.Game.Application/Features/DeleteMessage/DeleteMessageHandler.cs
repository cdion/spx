...
{
    public async Task<GameMessageCommandOutcome> HandleAsync(Guid gameId, Guid playerId, Guid messageId, CancellationToken cancellationToken = default)
    {
        var result = await gameMessagePersistence.DeleteMessageAsync(gameId, playerId, messageId, cancellationToken);
