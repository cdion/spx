...
) : IAccountIdentity
{
    ...
    public async Task<AccountPasswordSignInOutcome> PasswordSignInAsync(
        AccountUser user,
        string password
    )
    {
        ...
        {
            return new AccountPasswordSignInOutcome(
                AccountPasswordSignInStatus.EmailConfirmationRequired
            );
        }
    ...
    public async Task<AccountOperationOutcome> ResetPasswordAsync(
        AccountUser user,
        string code,
        string password
    )
    ...
    private static AccountOperationOutcome Map(IdentityResult result) =>
        result.Succeeded
            ? new AccountOperationSucceeded()
            : new AccountOperationFailed(MapErrors(result));

    private static AccountOperationFailed Failure(params string[] errors) =>
