...
public static class AccountEndpointRouteBuilderExtensions
{
    ...
    private static async Task<IResult> LoginAsync(HttpContext httpContext, ILoginHandler handler)
    {
        ...
        return Results.LocalRedirect(
            MapLoginOutcome(await handler.HandleAsync(email, password, form["returnUrl"]))
        );
    }
    ...
    private static async Task<IResult> LogoutAsync(ILogoutHandler handler)
    {
        ...
        return Results.LocalRedirect(
            BuildRedirect("/login", ("status", "You have been signed out."))
        );
    }
    ...
    {
        return Results.LocalRedirect(
            MapConfirmEmailOutcome(await handler.HandleAsync(userId, code))
        );
    }
    ...
    )
    {
        ...
        var email = GetRequiredValue(form, "email");
        return Results.LocalRedirect(
            MapForgotPasswordOutcome(await handler.HandleAsync(email, httpContext.RequestAborted))
        );
    }
    ...
    )
    {
        ...
        var confirmPassword = GetRequiredValue(form, "confirmPassword");
        return Results.LocalRedirect(
            MapResetPasswordOutcome(
                await handler.HandleAsync(email, code, password, confirmPassword)
            )
        );
    }
    ...
    private static async Task<IResult> ResendConfirmationAsync(
        HttpContext httpContext,
        IResendConfirmationHandler handler
    )
    {
        var form = await httpContext.Request.ReadFormAsync();
        var email = GetRequiredValue(form, "email");
        return Results.LocalRedirect(
            MapResendConfirmationOutcome(
                await handler.HandleAsync(email, httpContext.RequestAborted)
            )
        );
    }
    ...
    private static string MapLoginOutcome(LoginOutcome outcome) =>
        outcome.Status switch
        {
            ...
            LoginOutcomeStatus.ValidationFailed => BuildRedirect(
                "/login",
                ("error", "Email and password are required."),
                ("returnUrl", outcome.ReturnUrl)
            ),
            LoginOutcomeStatus.InvalidCredentials => BuildRedirect(
                "/login",
                ("error", "Invalid email or password."),
                ("returnUrl", outcome.ReturnUrl)
            ),
            LoginOutcomeStatus.EmailConfirmationRequired => BuildRedirect(
                "/login",
                ("error", "Confirm your email before signing in."),
                ("email", outcome.Email),
                ("returnUrl", outcome.ReturnUrl)
            ),
            LoginOutcomeStatus.LockedOut => BuildRedirect(
                "/login",
                ("error", "Your account is temporarily locked. Try again later."),
                ("returnUrl", outcome.ReturnUrl)
            ),
            _ => BuildRedirect(
                "/login",
    ...
    private static string MapRegisterOutcome(RegisterOutcome outcome) =>
        outcome.Status switch
        {
            RegisterOutcomeStatus.ValidationFailed => BuildRedirect(
                "/register",
                ("error", "Email, password, and confirmation are required.")
            ),
            RegisterOutcomeStatus.PasswordMismatch => BuildRedirect(
                "/register",
                ("error", "Passwords do not match."),
                ("email", outcome.Email)
            ),
            RegisterOutcomeStatus.Failed => BuildRedirect(
                "/register",
                ("error", string.Join(" ", outcome.Errors ?? [])),
                ("email", outcome.Email)
            ),
            RegisterOutcomeStatus.ConfirmationResendRequired => BuildRedirect(
                "/resend-confirmation",
                ("status", string.Join(" ", outcome.Errors ?? [])),
                ("email", outcome.Email)
            ),
            RegisterOutcomeStatus.ConfirmationSent => BuildRedirect(
                "/login",
                ("status", "Check your email to confirm your account."),
                ("email", outcome.Email)
            ),
            _ => BuildRedirect(
                "/register",
    ...
    private static string MapConfirmEmailOutcome(ConfirmEmailOutcome outcome) =>
        outcome.Status switch
        {
            ConfirmEmailOutcomeStatus.InvalidLink => BuildRedirect(
                "/login",
                ("error", "The confirmation link is invalid.")
            ),
            ConfirmEmailOutcomeStatus.Succeeded => BuildRedirect(
                "/login",
                ("status", "Your email has been confirmed. You can sign in now."),
                ("email", outcome.Email)
            ),
            ConfirmEmailOutcomeStatus.Failed => BuildRedirect(
                "/login",
                (
                    "error",
    ...
    private static string MapForgotPasswordOutcome(ForgotPasswordOutcome outcome) =>
        outcome.Status switch
        {
            ForgotPasswordOutcomeStatus.ValidationFailed => BuildRedirect(
                "/forgot-password",
                ("error", "Email is required.")
            ),
            ForgotPasswordOutcomeStatus.Completed => BuildRedirect(
                "/forgot-password",
                (
                    "status",
                    "If the account exists and the email is confirmed, a reset link has been sent."
                )
            ),
    ...
    private static string MapResetPasswordOutcome(ResetPasswordOutcome outcome) =>
        outcome.Status switch
        {
            ResetPasswordOutcomeStatus.IncompleteLink => BuildRedirect(
                "/reset-password",
                ("error", "The reset link is incomplete."),
                ("email", outcome.Email),
                ("code", outcome.Code)
            ),
            ResetPasswordOutcomeStatus.PasswordMismatch => BuildRedirect(
                "/reset-password",
                ("error", "Passwords do not match."),
                ("email", outcome.Email),
                ("code", outcome.Code)
            ),
            ResetPasswordOutcomeStatus.Completed => BuildRedirect(
                "/login",
                ("status", "Your password has been reset. You can sign in now."),
                ("email", outcome.Email)
            ),
            ResetPasswordOutcomeStatus.Failed => BuildRedirect(
                "/reset-password",
                ("error", string.Join(" ", outcome.Errors ?? [])),
                ("email", outcome.Email),
                ("code", outcome.Code)
    ...
    private static string MapResendConfirmationOutcome(ResendConfirmationOutcome outcome) =>
        outcome.Status switch
        {
            ResendConfirmationOutcomeStatus.ValidationFailed => BuildRedirect(
                "/resend-confirmation",
                ("error", "Email is required.")
            ),
            ResendConfirmationOutcomeStatus.Completed => BuildRedirect(
                "/resend-confirmation",
                (
                    "status",
                    "If the account exists and is still unconfirmed, a new confirmation email has been sent."
                ),
                ("email", outcome.Email)
            ),
