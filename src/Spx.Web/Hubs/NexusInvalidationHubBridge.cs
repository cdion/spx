...
) : ILobbyInvalidationObserver, IGameInvalidationHubBridge, IHostedService
{
    ...
    public async Task OnGameConnectedAsync(Guid gameId)
    {
        ...
        if (newCount == 1)
        {
            await clusterClient.GetGrain<ILobbyInvalidationGrain>(gameId).Subscribe(observerRef!);
        }
    }
    ...
    public async Task OnGameDisconnectedAsync(Guid gameId)
    {
        ...
        if (newCount == 0)
        {
            ...
            await clusterClient.GetGrain<ILobbyInvalidationGrain>(gameId).Unsubscribe(observerRef!);
        }
    }
