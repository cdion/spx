...
public sealed partial class LobbyInvalidationGrain(ILogger<LobbyInvalidationGrain> logger)
    : Grain,
        ILobbyInvalidationGrain
{
    private readonly HashSet<ILobbyInvalidationObserver> observers = [];

    public Task Subscribe(ILobbyInvalidationObserver observer)
    {
        observers.Add(observer);
        return Task.CompletedTask;
    }

    public Task Unsubscribe(ILobbyInvalidationObserver observer)
    {
    ...
    private static void NotifyObservers(
        Guid gameId,
        ICollection<ILobbyInvalidationObserver> observers,
        Action<ILobbyInvalidationObserver, Guid> notifyObserver,
        ILogger<LobbyInvalidationGrain> logger
    )
    {
        List<ILobbyInvalidationObserver>? disconnectedObservers = null;
